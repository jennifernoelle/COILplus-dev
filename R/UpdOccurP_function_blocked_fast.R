...
#' @name UpdOccurP_blocked_fast
#' @title Blocked Metropolis-Hastings update of occurrence probabilities and indicators
#'
...
#'
#' @param mh_p_step Scalar. Standard deviation of the truncated-normal
#'   Metropolis-Hastings proposal for the occurrence probability p.
#' @param mh_pprior_sd Scalar. Standard deviation of the truncated-normal
#'   prior on p. Used to compute the log-prior density of proposed values.
#' @param p_1to0 Scalar. Bernoulli proposal probability for transitioning an
#'   occurrence indicator from 1 to 0 at each step.
#' @param p_0to1 Scalar. Bernoulli proposal probability for transitioning an
#'   occurrence indicator from 0 to 1 at each step.
#' @param occ_curr Matrix. Rows are species, columns are studies. Current
#'   binary occurrence indicators (0/1) for the species being updated.
#'   Updated values are returned in the output list.
#' @param logZ_prior_mat Optional matrix with the same dimensions as `p_curr`.
#'   Precomputed log-normalizing constants for the truncated-normal prior on p,
#'   one value per species per study. If `NULL`, computed internally each call.
